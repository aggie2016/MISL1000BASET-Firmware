...
void wizCmd(unsigned char cmd) { //send a command to socket 0 and wait for completion
	...
	while (wizRead(SnCR, WIZNET_READ_S0R, &cmd, 1), cmd)
		; //wait til command completes
}
...
public:
	...
	void Write(unsigned int addr, unsigned char opcode, void * data, unsigned int len)
	{
		// Activate the CS pin
		enablewiz();   			
		spiSendN((unsigned char *)&addr, 2);
		// Send Wiznet Write OpCode
		spiSend(opcode);
		//Send the payload
		spiSendN(data, len);
	...
	void Read(unsigned int addr, unsigned char opcode, void * data, unsigned int len)
	{
		// Activate the CS pin
		enablewiz();
		spiSendN((unsigned char *)&addr, 2);
		// Send Wiznet Read OpCode
		spiSend(opcode);   		
		// Send the data byte
		spiReceiveN(data, len);	
		// make CS pin not active
		disablewiz();				
	}
	...
	{
		//send the command
		wizWrite(SnCR, WIZNET_WRITE_S0R, &cmd, 1); 
		//wait til command completes
		while (wizRead(SnCR, WIZNET_READ_S0R, &cmd, 1), cmd); 
	}
	void wizSetCtl8(unsigned int ctlreg, unsigned char val)
	{
		wizWrite(ctlreg, WIZNET_WRITE_S0R, &val, 1);
	}
	unsigned char wizGetCtl8(unsigned int ctlregaddr)
	...
	void socket0_init() { //initialize socket 0 for http server
		SendCommand(CR_CLOSE); //make sure port is closed
		wizSetCtl8(SnIR, 0xFF); //reset interrupt reg
		wizSetCtl8(SnMR, MR_TCP); //set mode register to tcp
		wizSetCtl16(SnPORT, 80); //set tcp port to 80
		SendCommand(CR_OPEN); //open the port
		SendCommand(CR_LISTEN); //listen for a conection
	}
	unsigned int send0(unsigned char *buf, unsigned int buflen) {
		unsigned int timeout, txsize, txfree;
		unsigned char crsend = CR_SEND, crreadback;
		...
		wizWrite(txwr, WIZNET_WRITE_S0TX, buf, buflen); //write the outgoing data to the transmit buffer
		wizSetCtl16(SnTX_WR, txwr + buflen);//update the buffer pointer
		SendCommand(CR_SEND); // Now Send the SEND command which tells the wiznet the pointer is updated
		return 1;
	}
	...
	void flush(unsigned int rsize) { //this just gets rid of data that i don't want to process
		...
		if (rsize > 0) {
			rxrd = wizGetCtl16(SnRX_RD); //retrieve read data pointer
			wizSetCtl16(SnRX_RD, rxrd + rsize); //replace read data pointer
			SendCommand(CR_RECV); //tell the wiznet we`ve retrieved the data
		}
	}
